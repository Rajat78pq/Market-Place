...
export class HorizontaltopbarComponent implements OnInit, AfterViewInit {
  ...
  private activateMenu() {
    ...
    // tslint:disable-next-line: prefer-for-of
    for (let i = 0; i < links.length; i++) {
      const link = links[i] as HTMLAnchorElement; // Cast to HTMLAnchorElement
  ...
  _activateMenuDropdown() {
    ...
    if (menuItemEl) {
      ...
      const activateParent = (el: HTMLElement | null) => {
        if (el && el.id !== 'side-menu') {
          el.classList.add('mm-active');
          const childAnchor = el.querySelector('.has-arrow');
          const childDropdown = el.querySelector('.has-dropdown');
      ...
      let currentEl: HTMLElement | null = menuItemEl.parentElement;
      while (currentEl) {
        currentEl = activateParent(currentEl);
        if (currentEl) {
